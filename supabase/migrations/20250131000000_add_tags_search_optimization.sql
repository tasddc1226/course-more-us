...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_places_tags_gin 
ON places USING GIN (tags);

-- 2. Full Text Search 성능 향상을 위한 인덱스
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_places_fts 
ON places USING GIN (to_tsvector('korean', coalesce(name, '') || ' ' || coalesce(description, '')));

-- 3. 태그 개별 요소 검색을 위한 인덱스 (부분 문자열 검색)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_places_tags_elements 
ON places USING GIN (tags gin_trgm_ops);

-- 4. 활성 장소 + 태그 복합 인덱스
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_places_active_tags 
ON places (is_active) WHERE is_active = true;

-- 5. 지역별 태그 검색을 위한 복합 인덱스
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_places_region_tags 
